...
# MAGIC
# MAGIC ## Tables
# MAGIC | Table | Model | Description |
# MAGIC |-------|-------|-------------|
# MAGIC | `demand_predictions` | `load_forecast_nsw@champion` | NSW demand forecast vs actual |
# MAGIC | `price_predictions` | `price_forecast_nsw@champion` | NSW price forecast vs actual |
# MAGIC
# MAGIC If no model is registered yet (first run before workshop), tables are created
# MAGIC empty with the correct schema so the Dash app can fall back to sample data.
...
def demand_predictions():
    ...
    # Align columns with model's expected schema (fill missing with 0)
    try:
        model_input_schema = model.metadata.get_input_schema()
        if model_input_schema is not None:
            expected_cols = [col.name for col in model_input_schema.inputs]
